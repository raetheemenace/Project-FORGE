...
router.get('/analytics', authenticateToken, requireRole('LAB_ADMIN'), async (req, res) => {
  ...
  if (dateFrom)   { conditions.push(`report_date >= $${idx++}`); params.push(dateFrom); }
  if (dateTo)     { conditions.push(`report_date <= $${idx++}`); params.push(dateTo); }

  const where = conditions.length ? `WHERE ${conditions.join(' AND ')}` : '';

  try {
    const stored = await db.query(
      `SELECT report_date, department, total_transactions, total_equipment_borrowed,
              total_maintenance_reports, avg_session_duration
       FROM forge_analytics_daily ${where}
    ...
    if (department) { liveConditions.push(`t.department = $${liveIdx++}`); liveParams.push(department); }
    if (dateFrom)   { liveConditions.push(`t.txn_date >= $${liveIdx++}`); liveParams.push(dateFrom); }
    if (dateTo)     { liveConditions.push(`t.txn_date <= $${liveIdx++}`); liveParams.push(dateTo); }
    ...
    const live = await db.query(
      `SELECT
       ...
       LEFT JOIN forge_maintenance m
         ON m.equipment_id IN (SELECT equipment_id FROM forge_txn_items WHERE txn_id = t.txn_id)
...
router.get('/audit-log', authenticateToken, requireRole('LAB_ADMIN'), async (req, res) => {
  ...
  if (adminId)     { conditions.push(`a.admin_id = $${idx++}`); params.push(parseInt(adminId, 10)); }
  if (actionType)  { conditions.push(`a.action_type ILIKE $${idx++}`); params.push(`%${actionType}%`); }
  if (targetType)  { conditions.push(`a.target_type = $${idx++}`); params.push(targetType); }
  ...
  try {
    const result = await db.query(
      `SELECT a.action_id, a.admin_id, u.full_name AS admin_full_name,
              a.action_type, a.target_type, a.target_id, a.details, a.created_at
    ...
    const countResult = await db.query(
      `SELECT COUNT(*) AS total FROM forge_admin_actions a
       JOIN forge_users u ON u.user_id = a.admin_id ${where}`,
      params
    );
